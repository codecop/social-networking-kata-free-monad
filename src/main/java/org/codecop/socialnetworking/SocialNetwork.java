...
public class SocialNetwork {
    ...
    public static void main(String[] args) throws IOException {
        Free<DslCommand<Void>, Void> app = app();
        System.err.println(app);
        Interpret.it(app);
    }

    static Free<DslCommand<Void>, Void> app() {
        Free<DslCommand<BufferedReader>, BufferedReader> init = // 
    ...
    static Free<DslCommand<Void>, Void> processInput(Free<DslCommand<BufferedReader>> inputCmd) {
        ...
        Function<DslCommand<Command>, DslCommand<Free<DslCommand<Void>, Void>>> f4 = named("processCommand",
                commandCmd -> processCommand(inputCmd, commandCmd));
        Function<Free<DslCommand<Command>>, Free<DslCommand<Free<DslCommand<Void>, Void>>>> f5 = named(
    ...
    static DslCommand<Free<DslCommand<Void>, Void>> processCommand(Free<DslCommand<BufferedReader>> inputCmd,
            DslCommand<Command> commandCmd) {
        return commandCmd.map(command -> processCommand(inputCmd, command));
    }

    private static Free<DslCommand<Void>, Void> processCommand(Free<DslCommand<BufferedReader>> inputCmd,
            Command command) {
        if ("quit".equalsIgnoreCase(command.line)) {
            return Free.liftF(DslResult.nil());
        }

        Free<DslCommand<Void>, Void> result = Commands.handle(command);
        Free<DslCommand<Void>, Void> remaining = 
