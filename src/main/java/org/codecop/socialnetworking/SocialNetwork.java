...
public class SocialNetwork {
    ...
    private static Free<DslCommand<Void>> processCommand(Free<DslCommand<BufferedReader>> inputCmd,
            Command command) {
        if ("quit".equalsIgnoreCase(command.line)) {
            return Free.liftF(DslResult.nil());
        }

        Free<DslCommand<Void>> result = Commands.handle(command);
        Free<DslCommand<Void>> remaining = 
